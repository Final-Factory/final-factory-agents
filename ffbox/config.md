...
| `machine_id` | `46696e616c466163746f72792d666662` | What Unity's licensing service thinks this machine is. The default is our constant ("FinalFactory-ffb"), which `ffbox/unity-offline-license.sh` mints the licence against — keep it in lockstep with `FFBOX_MACHINE_ID_CONST` there. `image` leaves the image's baked-in value alone. `per-slot` is the old default and is correct only for a lane that has gone back to online activation; a `.ulf` binds to exactly one id, so per-slot now matches nothing and finds no entitlement. |
