...
be a file path posted where it should not be — or, since the gate started reading `venue` too,
a reply posted into a conversation between two people who had not asked for one. Both fall closed when omitted, and `ffwatch`
...
"rate_limits": {
  ...
  "operator": null,
  "users": {
    "somebody":     { "discord": "123456789012345678", "limit": 40 },
    "somebody_else": { "discord": "876543210987654321", "limit": 0 }
  },
  "send": { "per_hour": 60, "per_conversation_hour": 12 }
}
```
...
The connection limits are ffintake's own and deliberately not configurable: 32 connections at
once and 4 per sender, 10 new connections a minute per sender, 15 seconds to finish the TLS
handshake and headers, 180 seconds for the whole connection, 20 for any single read, and
headers capped at 32 lines of 8 KB. A `max_file_mb` key from the first version is ignored.
