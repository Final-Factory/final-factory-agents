...
class Watcher:
    def __init__(self, cfg, dry_run=False):
        ...
        # THE CLAUDE ACCOUNT READER. It caches each key's windows for `refresh_secs`, which is
        # the whole of the rate-limiting on this: callers ask whenever they need an answer and
        # the requests happen a quarter-hour apart.
    ...
    def pool_stage(self, agent_class=None):
        ...
        the TTL and the network come from that class's block, so a class staged on another branch,
        retiring on another schedule or fenced differently needs no code here.

        THE NETWORK HAS TO BE SET HERE and not at dispatch: a container's network is fixed when
        it is created, and dispatch renames a container that already exists. A staged ffdev
        container is on the open bridge from the moment it is staged, which is fine because
        pool_claim_for will only ever hand it to an ffdev turn.
        """
    ...
    def pool_claude_key(self, pool_id):
        ...
        None covers a spare staged before this was written down and a box with one account, and
        both mean the same thing to the caller: nothing was chosen, so nothing is recorded.
        """
        try:
            with open(os.path.join(self.pool_dir(pool_id), "claude-key"),
                      encoding="utf-8") as fh:
    ...
    def launch(self, turn_id):
        ...
        if claude_key and not pool_id:
            ...
            # of the call.
            #
            # COLD RUNS ONLY. A dispatch starts no container — it hands a job to one that has
        ...
        if pool_id:
            ...
            # pool_claim_for only hands back containers of this class, so the two routes agree.
            # Passing --network on the dispatch branch would be worse than useless: the container
