#!/usr/bin/env bash
#
# Unity activation and, more importantly, the return of the seat. SOURCED by the workflow's
# `run:` step, never executed:
#
#     . /opt/ffghr/unity-license.sh
...
# The editor is extremely chatty and this runs inside a job log that a human reads. Full output
# goes to a file; the tail is printed only when something failed, which is when it is wanted.
: "${FFGHR_LICENSE_LOG:=${RUNNER_TEMP:-/tmp}/unity-license.log}"
mkdir -p "$(dirname "$FFGHR_LICENSE_LOG")"

return_license() {
    local rc=$?
    if [ "$FFGHR_ACTIVATED" = 1 ]; then
        log "returning the Unity seat"
        if ! unity-editor -logFile /dev/stdout -quit -returnlicense \
                -username "$UNITY_EMAIL" -password "$UNITY_PASSWORD" \
                -projectPath /BlankProject >>"$FFGHR_LICENSE_LOG" 2>&1; then
            log "WARNING: the seat was not returned; it may be leaked"
            tail -30 "$FFGHR_LICENSE_LOG" >&2 || true
        else
            FFGHR_ACTIVATED=0
        fi
    fi
...
ensure_unity_license() {
    ...
    if ! activate_unity; then
        ...
        tail -50 "$FFGHR_LICENSE_LOG" >&2 || true
        exit 79
    fi
}
