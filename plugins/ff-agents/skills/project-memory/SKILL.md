...
- [Join/load-route provisioning is a desync class](memories/join-load-route-provisioning-desync-class.md) — six 055 forks, one shape: a load-time applier one group too early (R22), structural-change-inside-a-ComponentLookup-loop + paused-world systems silently no-op (R22b), a per-player component installed on only SOME arrival routes (R24), an allocator advancing by RESERVED instead of WRITTEN ids (R26), a controller-group write feeding a fixed-group read (R27), and a "heal on load" repair pass that must decline via `AppliesRemoteAuthoritativeWorld` on a remote snapshot apply (R28/R29)
