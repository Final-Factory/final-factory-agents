...
- [Machine-global state across checkouts & sessions](memories/machine-global-state-multi-session.md) — **LocalLow `Never Games/finalfactory/` (saves/, DeterminismAudit/, TestResults.xml) is keyed by product name, shared by EVERY checkout and every concurrent Claude session**; UDP 7777 admits one hoster machine-wide (SP LoadGame hosts too — bind failure = contention, retry, not a bug); MCP pins are per-session but subagents share the parent's pin; never rename/clear the shared dirs while a neighbor session may be live; close every editor you open (four editors + load loops = OOM)
