...
- [The ffbox updater restarts everything](memories/ffbox-updater-restarts-everything.md) — `ffbox-update.timer` fires ~every 5 min, fast-forwards the checkout, runs setup.sh stage 5 (`registerAgents.sh`, which is how a plugin bump lands) and restarts the whole `ffbox.target` including the Discord listener; after a push WAIT AND CHECK THE JOURNAL rather than restarting anything by hand — only systemd UNIT FILE changes need a human
